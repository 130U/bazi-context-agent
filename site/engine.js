...
export function buildLocalForecast(state, request = {}, runtimeConfig) {
  ...
  const hasAnnualOrLuckData = Boolean(selected.annual_fortunes?.length || selected.luck_cycles?.length);

  return {
    forecast_type: "honest_local_scenario",
    generated_for_date: currentDate,
    horizon: {
      months: horizonMonths,
      start_date: currentDate,
      end_date: addMonths(currentDate, horizonMonths)
    },
    selected_structure: selected,
    initial_conditions: contextFacts,
    question: typeof request.question === "string" ? request.question : "",
    scenario: {
      mode: hasAnnualOrLuckData ? "structured_local_scenario" : "degraded_context_planning_scenario",
      summary: hasAnnualOrLuckData
        ? "A deterministic local scenario anchored to the locked structure, supplied context, and requested time window."
        : "No annual-fortune or luck-cycle data is available in this browser build. This output is a context-aware planning scenario, not a calculated fate prediction.",
      focus_areas: contextFacts.map((fact) => ({ label: fact.label, value: copy(fact.value) })),
      structure_anchor: `${selected.hour_label} / ${selected.branch}`
    },
    limitations: hasAnnualOrLuckData
      ? ["Local deterministic scenario; no model-generated interpretation was used."]
      : [
          "Annual-fortune data is unavailable.",
          "Luck-cycle data is unavailable.",
          "Do not treat this planning scenario as a verified personal prediction."
        ],
    policy: {
